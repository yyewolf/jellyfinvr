...
```
init (2292)                     DOMContentLoaded
  └─ scan (2286)                MutationObserver on <html> subtree + 1s interval
       └─ addVrButton (2266)    injects "VR" button next to Jellyfin's fullscreen button
            └─ openPlayer (2111) ← click
                 ├─ findVideo (142)              pick Jellyfin's <video>
                 ├─ readJellyfinContext (173)    derive API credentials from the stream URL
                 ├─ buildCompatStreamUrl (450)   precompute H.264 fallback URL
                 ├─ loadItemText (426)           fetch item name/path for mode detection
                 ├─ ensureThree (137)             lazy-load Three.js from CDN
                 ├─ buildOverlay (520)           full-screen DOM overlay + 2D toolbar
                 ├─ buildRenderer (712)          scene, camera, WebGLRenderer, xr.enabled
                 │    ├─ environmentRoot        empty group for the room (section 4)
                 │    ├─ createPanel (554)       canvas-textured in-VR control panel
                 │    └─ setupControllers (1600)  2 controllers, laser beams, event wiring
                 ├─ bindMediaEvents (1876)
                 └─ replaceVideoTexture (1578) → rebuildVideoMeshes (1550)
                                              └─ syncEnvironment (1459) builds/shows the room
                                              ↓
                 autoDetectMode (391)         fires once dimensions + metadata are known
                                              ↓
                 enterVr (2066) ← "Enter VR" click → navigator.xr.requestSession
                                              ↓
                 renderFrame (1863)           setAnimationLoop, runs every XR frame
                                              ↓
                 closePlayer (2152)           tears everything down, restores <video>
```
...
Every mutation goes through `applyMode(changes, remember)` (line 385), which
...
**3.2 Geometry per eye** — `makeGeometry(projection, eye)` (line 1491):

- `360` → inverted sphere, full 2π sweep, radius 50.
- `180` → inverted half-sphere (`-π/2` start, π length), rotated `y = π/2` when placed.
- `flat` → plane sized and placed by `computeScreenLayout` (line 792). **This is
...
rebuild** via `rebuildVideoMeshes` (line 1550).
...
`configureEyeLayers` (line 1589) runs every frame, grabs `renderer.xr.getCamera()`,
...
`environmentActive()` (line 765) is the gate: an environment only renders when
...
reverse also holds — `applyEnvironment` (line 1474) switches the projection to
...
**Everything in the room is on layer 0.** `configureEyeLayers` (line 1589)
...
`computeScreenLayout` (line 792) replaced the hardcoded 7.2 × 4.05 plane. It
...
`videoAspect` (line 775) is the subtle part. **Pixel dimensions alone do not
...
- **It never touches UVs**, which is what lets the stereo UV baking in
  `makeGeometry` (line 1491) run afterwards exactly as before.
...
`buildTheater` (line 1334) assembles five pieces, all authored with the floor at
y = 0 and the viewer standing at the origin.

`buildFloor` (line 1135) is the load-bearing one. The floor is a **stadium
...
`buildSeating` (line 1110) places 308 seats. Rows arc around a point 110 m
...
`buildShell` (line 1161) is the room box plus the stage apron and two screen
speaker stacks. `buildFixtures` (line 1227) adds wall sconces, aisle markers,
...
cannot import, so `mergeParts` (line 972) does the merge by hand — converting to
...
- `buildRenderer` (line 712) creates the empty `environmentRoot`.
- `rebuildVideoMeshes` (line 1550) recomputes `screenLayout` and calls
  `syncEnvironment` (line 1459) *before* the video-texture guard, so the room
  tracks mode changes even before a texture exists.
- `buildEnvironment` (line 1445) is idempotent via `environmentBuilt`; the room
...
`resolveMode` (line 341) picks a projection/stereo pair from three tiers, highest
priority first:

1. **Saved override** for the Jellyfin item id — `readStoredModes` (line 361) /
   `rememberMode` (line 369), in `localStorage` under `jvr.modes.v1`, capped at
   200 entries with oldest-first eviction.
2. **Markers in the item name or file path** — `detectFromText` (line 313)
   tokenises on non-alphanumerics and matches `PROJECTION_TOKENS` /
   `STEREO_TOKENS`. `stereoFromToken` (line 307) additionally strips `half`,
...
3. **Aspect ratio** — `detectFromAspect` (line 325): 4:1 → 360/SBS,
...
Because the stream URL carries no file name, `loadItemText` (line 426) fetches
...
back to `document.title`. `autoDetectMode` (line 391) waits for both the video
...
`readJellyfinContext` (line 173) derives everything needed to call the API —
...
ApiClient's method signatures. `jellyfinRequest` (line 200) and
`jellyfinGetJson` (line 412) are the two transports; both authenticate with an
...
- `buildCompatStreamUrl` (line 450) rewrites `/Videos/{id}/stream` to
...
- `startCompatAt(position)` (line 1919) creates a 2×2 px `#jvr-compat-video`,
...
- Because the transcode starts at an offset, all time math goes through
  `getCurrentTime()` (line 481) = `compatOffset + video.currentTime`, and
  `seekAbsolute` (line 2003) restarts the transcode whenever the target falls
...
`<video>` is parked, so `reportProgress` (line 220) POSTs to
...
That is a safety property, not cosmetics: `stopEncoding` (line 261) issues
...
The control panel is a 1600×720 `<canvas>` painted by `drawPanel` (line 653) and
mapped onto a 1.82 × 0.82 m plane **parented to the camera**, so it is
head-locked until dragged.

- Hit-testing is UV-based: `raycastPanel` (line 1634) → `panelActionFromIntersection`
  (line 1643) converts `intersection.uv` to canvas pixels and matches against the
  `panelButtons` rect list that `addPanelButton` (line 621) rebuilds on every draw.
- Three interaction zones share the trigger, disambiguated in `beginTrigger`
  (line 1689): the timeline strip → scrub; the 62 px border → drag the panel in
  space; anything else → button click, or when the panel is hidden,
  drag-to-rotate the video sphere (`videoRoot.rotation`).
- Grip toggles the panel; holding grip 1200 ms calls `resetAll` (line 1851).
- `pollGamepads` (line 1812) reads raw `xrSession.inputSources`: button 4 = play/pause,
...
VR, handled by `handleToolbar` (line 2098). It duplicates the panel's actions.

UI strings are English. There is no i18n layer, so the Chinese-locale selectors
in `locateControls` (line 2259) are load-bearing — they are how the VR button
...
- `closePlayer` (line 2152) must keep restoring `currentTime`, `muted`, `volume`
